...
namespace TfL.Automation.UITests.Utils
{
    ...
    public sealed class Hooks : DriverManager
    {
        ...
        public static ConfigData GetConfigurationData()
        {
            var configData = new ConfigData();
            var iConfig = GetConfig();
            iConfig.Bind(configData);
            return configData;
        }

        private static IConfiguration GetConfig()
        {
            var builder = new ConfigurationBuilder().AddNewtonsoftJsonFile("configuration.json").Build();
            return builder;
        }
    }
}
